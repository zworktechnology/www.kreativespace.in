<?php include('header.php') ?>


<!-- PAGE TITLE
        ================================================== -->
<section class="page-title-section bg-img cover-background top-position1 secondary-overlay" data-overlay-dark="7" data-background="img/banner/page-title1.jpg">
    <div class="container">
        <div class="row">
            <div class="col-md-12 text-center">
                <div class="position-relative">
                    <h1>About Us</h1>
                </div>
                <ul>
                    <li><a href="index.php">Home</a></li>
                    <li><a href="#!">About Us</a></li>
                </ul>
            </div>
        </div>
    </div>
</section>

<!-- ABOUT
        ================================================== -->
<section class="overflow-visible">
    <div class="container position-relative z-index-3">
        <div class="row align-items-center about-style-02">
            <div class="col-lg-6 mb-1-9 mb-lg-0 wow fadeIn" data-wow-delay="200ms">
                <div class="me-xxl-2-9 position-relative pb-sm-12 text-center text-sm-start">
                    <img src="img/content/about4.jpg" class="border-radius-10" alt="...">
                    <div class="right bottom-0 d-none d-sm-block position-absolute">
                        <img src="img/content/about3.png" class="border-radius-10" alt="...">
                    </div>
                    <span class="line-box1"></span>
                    <span class="line-box2 d-none d-sm-block"></span>
                </div>
            </div>
            <div class="col-lg-6 wow fadeIn" data-wow-delay="400ms">
                <div class="ps-xl-1-9">
                    <div class="section-title">
                        <span class="sm-title">About Us</span>
                    </div>
                    <h2 class="h1 mb-1-9">We achieved more milestones - Here one of its,</h2>
                    <p class="mb-1-9">
                        At Kreative Space, we've achieved significant milestones, none more emblematic than completing a construction project totaling 25,000 square feet for Buhari Chain of Restaurants in Chennai.
                        However, our journey doesn't halt here. With unwavering dedication, we continue to surpass expectations, delivering excellence in every endeavor.
                    </p>
                    <p class="mb-1-9">
                        From conceptualization to realization, our commitment to quality and client satisfaction remains steadfast, driving us toward even greater accomplishments in the future. </p>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- WHY CHOOSE US
        ================================================== -->
<section class="bg-img cover-background pb-0 pt-lg-0 secondary-overlay" data-overlay-dark="8" data-background="img/bg/bg-02.jpg">
    <div class="container position-relative z-index-9">
        <div class="row align-items-center">
            <div class="col-lg-6 mb-2-9 mb-lg-0 wow fadeIn" data-wow-delay="200ms">
                <div class="story-video">
                    <div class="section-title mb-1-9">
                        <span class="sm-title">Why Choose Us</span>
                        <h2 class="mb-0 h1 text-white">Reason for people choosing Kreative Space</h2>
                    </div>
                    <!-- <a class="video video_btn" href="https://www.youtube.com/watch?v=x4dEfxj_Ttk">
                        <i class="fa fa-play"></i>
                    </a> -->
                </div>
            </div>
            <div class="col-lg-6 wow fadeIn" data-wow-delay="400ms">
                <div class="ps-xl-6">
                    <ul class="why-us-block">
                        <li class="inner-block">
                            <div class="content">
                                <img src="img/icons/13.png" class="mb-3" alt="...">
                                <h4>Modern Technology</h4>
                            </div>
                        </li>
                        <li class="inner-block color2">
                            <div class="content">
                                <img src="img/icons/14.png" class="mb-3" alt="...">
                                <h4>Timely Completion</h4>
                            </div>
                        </li>
                        <li class="inner-block color2">
                            <div class="content">
                                <img src="img/icons/15.png" class="mb-3" alt="...">
                                <h4> Cost - Effective</h4>
                            </div>
                        </li>
                        <li class="inner-block">
                            <div class="content">
                                <img src="img/icons/16.png" class="mb-3" alt="...">
                                <h4>Expert Engineer</h4>
                            </div>
                        </li>
                        <li class="inner-block">
                            <div class="content">
                                <img src="img/icons/01.png" class="mb-3" alt="...">
                                <h4>Awards winner</h4>
                            </div>
                        </li>
                        <li class="inner-block color2">
                            <div class="content">
                                <img src="img/icons/02.png" class="mb-3" alt="...">
                                <h4>Best Quality</h4>
                            </div>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- PROCESS
        ================================================== -->
<section>
    <div class="container">
        <div class="section-title mb-1-9 mb-md-6 text-center wow fadeIn" data-wow-delay="200ms">
            <span class="sm-title">Work Process</span>
            <h2 class="mb-0 h1">Our Working Process</h2>
        </div>
        <div class="text-center">
            <div class="row mt-n1-9">
                <div class="col-lg-3 col-md-6 mt-1-9 wow fadeIn" data-wow-delay="400ms">
                    <div class="process-style1 position-relative">
                        <div class="process-icon">
                            <div class="process-img">
                                <img src="img/icons/17.png" alt="...">
                                <div class="tag">01</div>
                            </div>
                        </div>
                        <h4 class="mb-0">Client Needs</h4>
                        <img src="img/content/arrow01.png" class="arrow" alt="...">
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 mt-1-9 wow fadeIn" data-wow-delay="600ms">
                    <div class="process-style1 position-relative">
                        <div class="process-icon">
                            <div class="process-img">
                                <img src="img/icons/18.png" alt="...">
                                <div class="tag">02</div>
                            </div>
                        </div>
                        <h4 class="mb-0">Planning Design</h4>
                        <img src="img/content/arrow01.png" class="arrow" alt="...">
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 mt-1-9 wow fadeIn" data-wow-delay="800ms">
                    <div class="process-style1 position-relative">
                        <div class="process-icon">
                            <div class="process-img">
                                <img src="img/icons/19.png" alt="...">
                                <div class="tag">03</div>
                            </div>
                        </div>
                        <h4 class="mb-0">Architect Sketch</h4>
                        <img src="img/content/arrow01.png" class="arrow" alt="...">
                    </div>
                </div>
                <div class="col-lg-3 col-md-6 mt-1-9 wow fadeIn" data-wow-delay="1000ms">
                    <div class="process-style1 position-relative">
                        <div class="process-icon">
                            <div class="process-img">
                                <img src="img/icons/20.png" alt="...">
                                <div class="tag">04</div>
                            </div>
                        </div>
                        <h4 class="mb-0">Start Bulding</h4>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- TESTIMONIAL
        ================================================== -->
<section class="bg-light bg-img cover-background" data-background="img/bg/bg-07.png">
    <div class="container">
        <div class="section-title mb-1-9 mb-md-6 text-center wow fadeIn" data-wow-delay="200ms">
            <span class="sm-title">Testimonial</span>
            <h2 class="mb-0 h1">What Our Client Say</h2>
        </div>
        <div class="row gx-xxl-5 mt-n2-9">
            <div class="col-md-6 col-lg-4 mt-2-9 wow fadeIn" data-wow-delay="400ms">
                <div class="testimonial-style1">
                    <div class="d-flex">
                        <div class="flex-shrink-0">
                            <img src="img/avatars/avatar-1.jpg" class="border-radius-10" alt="...">
                        </div>
                        <div class="flex-grow-1 ms-4 pt-4">
                            <h4 class="h5 mb-1">Happy Customer</h4>
                            <small>Review</small>
                        </div>
                    </div>
                    <div class="testi-text" style="margin-top: 5px;">
                        <i class="fa-solid fa-quote-right icon"></i>
                        <div class="item-desc">
                            Choosing Kreative Space Construction was the best decision for our commercial project. Their innovative approach and timely delivery exceeded our expectations. Highly recommended!
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-6 col-lg-4 mt-2-9 wow fadeIn" data-wow-delay="600ms">
                <div class="testimonial-style1">
                    <div class="d-flex">
                        <div class="flex-shrink-0">
                            <img src="img/avatars/avatar-1.jpg" class="border-radius-10" alt="...">
                        </div>
                        <div class="flex-grow-1 ms-4 pt-4">
                            <h4 class="h5 mb-1">Happy Customer</h4>
                            <small>Review</small>
                        </div>
                    </div>
                    <div class="testi-text" style="margin-top: 5px;">
                        <i class="fa-solid fa-quote-right icon"></i>
                        <div class="item-desc">
                            Their team was fantastic to work with. They listened to my ideas and preferences, and their attention to detail was commendable. The quality
                            of their construction work was outstanding, and they completed the project on time</div>
                    </div>
                </div>
            </div>
            <div class="col-md-6 col-lg-4 mt-2-9 wow fadeIn" data-wow-delay="800ms">
                <div class="testimonial-style1">
                    <div class="d-flex">
                        <div class="flex-shrink-0">
                            <img src="img/avatars/avatar-1.jpg" class="border-radius-10" alt="...">
                        </div>
                        <div class="flex-grow-1 ms-4 pt-4">
                            <h4 class="h5 mb-1">Happy Customer</h4>
                            <small>Review</small>
                        </div>
                    </div>
                    <div class="testi-text" style="margin-top: 5px;">
                        <i class="fa-solid fa-quote-right icon"></i>
                        <div class="item-desc">
                            Kreative Space Construction has truly set a benchmark in construction in Tamil Nadu. Their commitment to quality and timelines is commendable. I am a satisfied customer!
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- OUR TEAM
        ================================================== -->
<section>
    <div class="container">
        <div class="section-title mb-1-9 mb-md-6 text-center" data-wow-delay="200ms">
            <span class="sm-title">Team Members</span>
            <h2 class="mb-0 h1">Our Expert Team</h2>
        </div>
        <div class="row mt-n1-9">
            <div class="col-md-6 col-lg-3 mt-1-9 wow fadeIn" data-wow-delay="400ms">
                <div class="team-style01">
                    <div class="image">
                        <img src="img/team/team-01.jpg" alt="...">
                    </div>
                    <div class="text-center pt-4">
                        <h3 class="h5 mb-1"><a href="javascript:void(0)">Name</a></h3>
                        <p class="mb-0">Position</p>
                    </div>
                </div>
            </div>
            <div class="col-md-6 col-lg-3 mt-1-9 wow fadeIn" data-wow-delay="600ms">
                <div class="team-style01">
                    <div class="image">
                        <img src="img/team/team-02.jpg" alt="...">
                    </div>
                    <div class="text-center pt-4">
                        <h3 class="h5 mb-1"><a href="javascript:void(0)">Name</a></h3>
                        <p class="mb-0">Poistion</p>
                    </div>
                </div>
            </div>
            <div class="col-md-6 col-lg-3 mt-1-9 wow fadeIn" data-wow-delay="800ms">
                <div class="team-style01">
                    <div class="image">
                        <img src="img/team/team-03.jpg" alt="...">
                    </div>
                    <div class="text-center pt-4">
                        <h3 class="h5 mb-1"><a href="javascript:void(0)">Name</a></h3>
                        <p class="mb-0">Position</p>
                    </div>
                </div>
            </div>
            <div class="col-md-6 col-lg-3 mt-1-9 wow fadeIn" data-wow-delay="1000ms">
                <div class="team-style01">
                    <div class="image">
                        <img src="img/team/team-04.jpg" alt="...">
                    </div>
                    <div class="text-center pt-4">
                        <h3 class="h5 mb-1"><a href="javascript:void(0)">Name</a></h3>
                        <p class="mb-0">Position</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- EXTRA
        ==================================================
        <section class="bg-primary py-8 overflow-visible">
            <div class="container">
                <div class="row justify-content-xxl-end align-items-center">
                    <div class="col-lg-8 col-xl-6 mb-4 mb-lg-0 wow fadeIn" data-wow-delay="200ms">
                        <div class="section-title white">
                            <h2 class="mb-0 h1 text-white">For all your construction needs!</h2>
                        </div>
                    </div>
                    <div class="col-lg-3 wow fadeIn" data-wow-delay="400ms">
                        <div class="text-lg-end">
                            <a href="contact.php" class="butn white"><span>Contact Us</span></a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="left-img-block">
                <div class="left-img">
                    <img src="img/content/footer.jpg" alt="...">
                </div>
            </div>
        </section> -->

<?php include('footer.php') ?>