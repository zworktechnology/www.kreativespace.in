...
(function ($) {
  var Quform = function ($form, options) {
    ...
    self.errorMessage = function (html, showTitle) {
      ...
      if (showTitle !== false) {
        $message.append(
          $('<div class="quform-error-title"/>').html(options.errorTitle)
        );
      }
    ...
    self.scrollTo = function ($target) {
      if (
        ...
        $target &&
        $target.length &&
        $(window).scrollTop() >= $target.offset().top - options.scrollThreshold
      ) {
    ...
    self.submit = function () {
      ...
      if (window.grecaptcha) {
        ...
        if ($recaptcha.length && $recaptcha.data("size") === "invisible") {
          if ($recaptcha.data("version") === "v3") {
            grecaptcha
              .execute($recaptcha.data("recaptcha-id"), { action: "quform" })
              .then(function (token) {
              ...
              if (typeof options.success === "function") {
                ...
                var success = function () {
                  ...
                  if (options.successTimeout > 0) {
                    setTimeout(function () {
                      $successMessage
                        .fadeOut(options.successFadeOutSpeed)
                        .hide(0, function () {
                ...
                $.each(response.elementErrors, function (index, info) {
                  if (
                    ...
                    if ($element.length) {
                      ...
                      $errorList.append(
                        '<div class="quform-error">' + info.errors[0] + "</div>"
                      );
                      $element
